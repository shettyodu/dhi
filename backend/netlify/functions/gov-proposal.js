...
const { fetchScope, generateSection, staticVolumes, generateComplianceMatrix, generateInfoRequests } = require("../../lib/proposal");
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (body.action === "section") {
      const r = await generateSection(o, body.section, body.scopeText || "", body.pageLimit, body.answers || "");
      return json(r.status, r.json);
    }
    ...
    if (body.action === "questions") {
      const r = await generateInfoRequests(o, body.scopeText || "");
      return json(r.status, r.json);
    }
    return json(400, { error: "Unknown action (use scope | section | static | matrix | questions)" });
  } catch (e) {
