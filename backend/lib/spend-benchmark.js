...
function analyze(lines) {
  const rows = (Array.isArray(lines) ? lines : []).slice(0, 250).map((ln) => {
    ...
    const vendor = ln.vendor ? String(ln.vendor).slice(0, 80).trim() : null;
    ...
    if (!m) return { desc, qty, vendor, unit_price: price, matched: false };
    ...
    if (price / bench > 12 || bench / price > 12) {
      return { desc, qty, vendor, unit_price: price, matched: false };
    }
