/* =====================================================================
   ...
   Proceeds to the shared checkout via checkout.html?cart=supplies.
   ===================================================================== */
(function () {
  ...
  function card(p) {
    ...
    const priceTxt = p.p == null ? "Request quote" : money(p.p);
    const priceNote = p.p == null ? "pricing on request" : "list price / " + (p.unit || "unit");
    return `<div class="flex flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
      ...
      <div class="mt-3 flex items-baseline justify-between border-t border-slate-100 pt-3">
        <span class="font-bold ${p.p == null ? "text-slate-400" : "text-brand-900"} text-lg">${priceTxt}</span>
        <span class="text-[11px] text-slate-400">${priceNote}</span>
      </div>
      ...
      <aside class="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-2xl">
        ...
        <div class="flex-1 overflow-y-auto px-5 py-4">${items.length ? `<ul class="space-y-2">${items.map((l) => { const pl = l.p; return `
          <li class="rounded-lg border border-slate-200 p-3">
            <div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="text-sm font-semibold text-brand-900">${pl.t}</p>
              <p class="font-mono text-xs text-slate-400">${l.id}</p>
              <p class="mt-0.5 text-sm font-semibold ${pl.p == null ? "text-slate-400" : "text-cyan-700"}">${pl.p == null ? "Request quote" : money(pl.p)}</p></div>
        ...
        <div class="border-t border-slate-200 px-5 py-4">
          ${items.length ? `<div class="mb-3 rounded-lg bg-slate-50 p-3 text-sm"><div class="flex items-center justify-between"><span class="text-slate-600">Subtotal</span><span class="text-lg font-bold text-brand-900">${money(sub)}</span></div>
            <p class="mt-1 text-xs text-slate-400">${pricedLines.length} priced${quoteOnly ? ` · ${quoteOnly} quote-only` : ""} · excl. tax &amp; freight</p></div>` : ""}
          ${pricedLines.length ? `<a href="checkout.html?cart=supplies" class="mb-2 block rounded-lg bg-cyan-600 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-cyan-700">Proceed to checkout &rarr;</a>` : ""}
          ...
          <a href="contact.html?interest=${encodeURIComponent("Supplies, Textiles & Linens")}" class="block text-center text-xs text-slate-500 hover:text-cyan-700">or request a formal quote &rarr;</a>
        </div></aside>`;
