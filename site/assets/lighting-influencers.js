/* Lighting Influencer Program — sign-up handler. Posts to the serverless
   function, shows the assigned referral code + personalized catalog link.
   Endpoint pattern matches checkout.js / automotive-passport.js. */
(function () {
  const API_BASE =
  ...
  function toast(msg) {
    ...
    clearTimeout(toastT); toastT = setTimeout(() => t.classList.add("opacity-0"), 2400);
  }
  ...
  function resultCard(d) {
    return `<div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
      ...
      <p class="mt-3 text-sm text-slate-600">Your personalized catalog link:</p>
      ...
      <p class="mt-3 text-xs text-slate-500">Share this link. Sales completed with your code in the order earn your payout. Remember to add FTC disclosure ("#ad").</p>
    </div>`;
  }
  ...
  if (form) form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      email: $("inf-email").value.trim(),
      channel: $("inf-channel").value.trim(),
      audience: $("inf-audience").value.trim(),
      payoutNote: $("inf-payout").value.trim(),
    };
    const status = $("inf-status"), btn = $("inf-submit"), result = $("inf-result");
    if (!payload.name || !payload.email || !payload.channel) {
      status.className = "text-sm text-red-600"; status.textContent = "Name, email, and channel are required.";
      return;
    }
    ...
    try {
      ...
      if (r.ok && d.code) {
        status.textContent = "";
        result.innerHTML = resultCard(d); result.classList.remove("hidden");
        ...
        if (copy) copy.addEventListener("click", () => {
          ...
          else { link.select(); document.execCommand && document.execCommand("copy"); toast("Link copied"); }
        });
        toast("Welcome aboard!");
      } else {
        status.className = "text-sm text-red-600"; status.textContent = d.error || `Sign-up failed (HTTP ${r.status}).`;
      }
    } catch (err) {
      status.className = "text-sm text-red-600"; status.textContent = "Network error — please try again.";
    } finally { btn.disabled = false; }
  });

  // If already signed up on this device, show their code instead of the form.
  try {
    const saved = JSON.parse(localStorage.getItem(STORE) || "null");
    if (saved && saved.code && form) {
      const result = $("inf-result");
      result.innerHTML = resultCard(saved); result.classList.remove("hidden");
      form.classList.add("hidden");
      const copy = $("inf-copy");
      if (copy) copy.addEventListener("click", () => {
        const link = $("inf-link");
        if (navigator.clipboard) navigator.clipboard.writeText(link.value).then(() => toast("Link copied"));
      });
    }
  } catch (e) {}
})();
